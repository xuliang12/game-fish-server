...
public class AppCluster {
    ...
    public static void main(String[] args) {
        ...
        String name = "game-cluster";
        ProjectNameMapUtil.setName(name);

        if ("target".equals(file.getName())) {
            path = file.getPath() + File.separatorChar + name + File.separator + "config";
        } else {
            path = file.getPath() + File.separatorChar + name + File.separator + "target" + File.separatorChar + "config";
        }
        log.info("配置路径为：" + path);
